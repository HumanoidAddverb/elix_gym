...
class LeggedRobot(BaseTask):
    ...
    def create_sim(self):
        ...
        if mesh_type == 'plane':
            ...
            raise ValueError(f"Terrain mesh type '{mesh_type}' not recognised. Allowed: [None, plane, heightfield, trimesh].")
        self._create_envs()
    ...
    def _create_heightfield(self):
        ...
        hf_params.nb_rows = self.terrain.tot_cols
        hf_params.nb_columns = self.terrain.tot_rows
    ...
    def _terrain_height_at(self, points_xy):
        ...
        pts = (points_xy + self.terrain.cfg.border_size) / self.terrain.cfg.horizontal_scale
        px = pts[..., 0].long().clip(0, self.height_samples.shape[0] - 2)
        py = pts[..., 1].long().clip(0, self.height_samples.shape[1] - 2)
        heights = torch.min(torch.min(self.height_samples[px, py], self.height_samples[px + 1, py]),
